function bindNeo4jTags(opts) {
  ...
  function addTagsToDoc(data) {
    ...
    var query = data.tags.map(function (taguuid) {
      ...
      return {
        statement: 'MATCH (tag:Tag{uuid:{tag}.uuid}) ' +
  ...
  function docTags(uuid) {
    ...
    var query = [
      {
        statement: 'MATCH (tag:Tag)-[r:TAG]->(doc:Doc{uuid:{doc}.uuid}) ' +
                   'RETURN tag',
        parameters: {
          doc: {uuid: uuid}
        }
  ...
  function removeTagsFromDoc(data) {
    ...
    var query = data.tags.map(function (taguuid) {
      return {
        statement: 'MATCH (tag:Tag{uuid:{tag}.uuid})-[r:TAG]->(doc:Doc{uuid:{doc}.uuid}) ' +
                   'DELETE r',
        parameters: {
          doc: {uuid: data.uuid},
          tag: {uuid: taguuid}
  ...
  return {
    ...
    rawSuggestedTagCloud,
    naiveSuggestedTagCloud
  };
}
