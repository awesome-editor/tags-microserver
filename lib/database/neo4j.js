var bindNeo4jCommon    = require('./support/neo4j-common').bindNeo4jCommon,
    ...
    bindNeo4jSSA       = require('./support/neo4j-ssa').bindNeo4jSSA;
...
function bindNeo4j(opts) {
  ...
  const neo4jCommon = bindNeo4jCommon(_.merge({neo4jFormatter: neo4jFormatter}, opts));

  const dbClient = neo4jCommon.db;

  _.assign(opts, {dbClient: dbClient});

  var neo4jDoc = bindNeo4jDoc(_.merge({neo4jFormatter: neo4jFormatter}, opts));
  var neo4jTags = bindNeo4jTags(_.merge({neo4jFormatter: neo4jFormatter}, opts));
  var neo4jSSA = bindNeo4jSSA(_.merge({
    neo4jTags: neo4jTags,
    neo4jFormatter: neo4jFormatter,
    neo4jCommon: neo4jCommon
  }, opts));


  var formatCreate = function (results) {
